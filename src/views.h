...
class kAudioView: virtual public kRectView{
    public:
    kAudioView(){ noSliders = 6; autoArrange = true; }
...
class kBrowseArchive: virtual public kButtonTree{
    public:
        ...
        kBrowseArchive(){autoArrange=true;rows=1; createEvents();
          enableUpdate();
        }
...
vector<shared_ptr<kOutSender> >outs;
        kPlayView(){
video  = make_shared<ofVideoPlayer>();
video2 = make_shared<ofVideoPlayer>();
//ofAddListener(ofEvents.update,this,&kPlayView::update);
            createEvents(); autoArrange = false;
        }
